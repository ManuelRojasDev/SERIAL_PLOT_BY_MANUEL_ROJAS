...
public class SERIAL_PLOT_BY_MANUEL_ROJAS extends PApplet {
...
//////////////////// fix the variables on height change.... and also the baud port on height change    
  ...
  int[] plotcolor = {0xffFF0307,0xffB200FF, 0xff0EFF03, 0xffE1FF03, 0xff03E4FF  ,0xffFC03EC,0xff038EFC, 0xffB5FC03, 0xffFC9103,0,0,0,0,0,0};//plot color buffer
  ...
  public void button(int axis,int bnumber, int xpos, int ypos, int xsize, int ysize, String variablename, float variablevalue,float variable_max,float variable_min, int textsize, int textcolor, float pointsize){//axis 0 for x axis 1 for y axis
...
{    
          ...
          {// x coordinate formula
            ...
            if ( mouseX >= y_axis)
              {// x coord formula + values
               float xcoord =(variable_max*1.0f*(mouseX-y_axis))/((graphic[1]+graphic[0])-y_axis);
               textSize(20);
                text(xcoord, displaywidth-xpos, displaycoordinate[0]);
              }
              else
              {// x coord formula - values
                float xcoord = (variable_min*1.0f*(y_axis-mouseX))/(y_axis-graphic[0]);  
                textSize(20);
                text(xcoord, displaywidth-xpos, displaycoordinate[0]);
              }
          }
          ...
          {//y coordinate formula
            ...
            if ( mouseY <= x_axis)
              {// y coord formula + values
                float ycoord = (variable_max*1.0f*(x_axis-mouseY))/(x_axis-graphic[3]);
                textSize(20);
                text(ycoord, displaywidth-(displaywidth*1098/10000), displaycoordinate[0]);
              }
             else
              {// y coord formula - values
                float ycoord = (variable_min*1.0f*(mouseY-x_axis))/(graphic[2]-x_axis);
                textSize(20);
                text(ycoord, displaywidth-(displaywidth*1098/10000), displaycoordinate[0]);
              }
          }
      ...
      if (mouseX >= graphic[0] && mouseX <= graphic[1]+graphic[0] && mouseY >= graphic[3] && mouseY <= graphic[2] && mousePressed && mouseButton == LEFT)
        {//xcoord
          noStroke();fill(204);rect(displaywidth-xpos, displaycoordinate[1], (displaywidth*1098/10000), 30); textSize(textsize+15);fill(0);
            if ( mouseX > y_axis)
              {// x coord + values
               float xcoord =(defscale[0]*1.0f*(mouseX-y_axis))/((graphic[1]+graphic[0])-y_axis);
               textSize(20);
                text(xcoord, displaywidth-xpos, displaycoordinate[0]);
              }
              else
              {// x coord - values
                float xcoord = (defscale[1]*1.0f*(y_axis-mouseX))/(y_axis-graphic[0]); 
                textSize(20);
                text(xcoord, displaywidth-xpos, displaycoordinate[0]);
              }
          ...
          noStroke();fill(204);rect(displaywidth-(displaywidth*1098/10000), displaycoordinate[1], (displaywidth*1098/10000), 30); textSize(textsize+15);fill(0);
            if ( mouseY <= x_axis)
              {// y coord + values
                float ycoord = (defscale[2]*1.0f*(x_axis-mouseY))/(x_axis-graphic[3]);
                textSize(20);
                text(ycoord, displaywidth-(displaywidth*1098/10000), displaycoordinate[0]);
              }
             else
              {// y coord - values
                float ycoord = (defscale[3]*1.0f*(mouseY-x_axis))/(graphic[2]-x_axis);
                textSize(20);
                text(ycoord, displaywidth-(displaywidth*1098/10000), displaycoordinate[0]);
              }
          }
  ...
  if ( drawaxis[0] == 0 && drawaxis[3] == 1 && xplotnumber[0] == 0 && yplotnumber[0] > 0 || drawaxis[0] == 0 && drawaxis[3] == 1 && xplotnumber[0] > 0 && yplotnumber[0] == 0 || drawaxis[0] == 1 && xplotnumber[0] > 0 && yplotnumber[0] > 0)
      {
        ...
        x_axis = (((graphic[2]-graphic[3])*defscale[2])*1.0f/(defscale[2]-defscale[3]))+graphic[3] ;//(((graphic[2]-graphic[3])*defscale[0]) / (defscale[0]-defscale[1])) + graphic[3];
        y_axis = ((-1*graphic[1]*defscale[1]*1.0f) / (defscale[0]-defscale[1]))+graphic[0];
  ...
  if (xplotnumber[0] == 0 && yplotnumber[0] == 0 && axis == 1 && drawaxis[1] == 0  && buttonpressed[bnumber] == 1)///// the y values define the x axis
    {
      ...
      lines[0] = 0;lines[2] = 0;lines[3] = 0; lines[1] = 0;noStroke();fill(plotsetting[0]); rect(0, graphic[3],displaywidth-(displaywidth*2196/10000),displayheight-graphic[3]);
      //int[] plotsize = { graphic[1], graphic[2]-graphic[3]};// x , y size in pixels
      x_axis = (((graphic[2]-graphic[3])*variable_max*1.0f)/(variable_max-variable_min))+graphic[3];              
      noStroke();fill(255,0,0);
   ...
   if (yplotnumber[0] == 0 && xplotnumber[0] == 0 && axis == 0 && drawaxis[2] < 10  && buttonpressed[bnumber] == 1 )///// the x values define the y axis
     {
       lines[0] = 0;lines[2] = 0;lines[3] = 0; lines[1] = 0;noStroke();fill(plotsetting[0]); rect(0, graphic[3],displaywidth-(displaywidth*2196/10000),displayheight-graphic[3]);
       //int[] plotsize = { graphic[1], graphic[2]-graphic[3]};// x , y size in pixels
       y_axis = (-1*(graphic[1]*variable_min*1.0f)/(variable_max-variable_min))+graphic[0];                       
        noStroke();fill(255,0,0);
  ...
  public void button2(int bnumber, int xpos, int ypos, int xsize, int ysize, int textsize){
...
public void Settings(int number,int xbegin,int ybegin,int xsize,int ysize){
  ...
  int clicked = 0;
  public void mouseClicked() {clicked = 1;}///mouse clicked
  ...
  public void Settingtext( int number, int xbegin, int ybegin, int xsize,int ysize, String text,int textsize, int Color){
...
public void serialport(int number, int xbegin, int ybegin, int xsize, int ysize, String text){
...
int selectedbaud = 9600;/// selected baud name , 9600 as default
  ...
  public void baudrate(int number, int xbegin, int ybegin, int xsize, int ysize, int text){
...
public void serialEvent( Serial Port){
  ...
  public void eSettings(int number, int xbegin, int ybegin, int xsize, int ysize, String text){/// extra setting for pot thickness, bg color, plotsetting[1]
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "SERIAL_PLOT_BY_MANUEL_ROJAS" };
    if (passedArgs != null) {
